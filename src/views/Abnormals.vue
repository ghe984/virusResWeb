      ...
      <el-card class="box-card">
        <div>
          <el-form :model="ruleForm"  ref="ruleForm" :inline="true" label-width="120px" class="demo-ruleForm">
            <el-form-item label="异常人员编号" prop="abnormal" :rules="[
      { required: true, message: '异常人员编号不能为空'},]">
              <el-input v-model="ruleForm.abnormal"></el-input>
            </el-form-item>
            <el-form-item label="症状" prop="abcondition" :rules="[
      { required: true, message: '症状不能为空'},]">
              <el-input style="width: 400px" placeholder="请输入症状" v-model="ruleForm.abcondition" ></el-input>
            </el-form-item>
                          <el-form-item label="登记者" prop="registrant" margin-bottom="10px">
                            <el-input style="width: 200px" placeholder="请输入登记者编号" v-model="ruleForm.registrant"></el-input>
                          </el-form-item>
            <el-form-item>
      ...
      <div>
        <div v-if="tableData !== undefined">
          <el-table
            ...
            >
              <el-switch
                  :disabled="scope.row.ishealed === 1"
                  v-model="scope.row.ishealed"
                  active-color="#13ce66"
...
<script>
import {deleteAbn, getAbnList, postAbno, putAbno, searchAbnoID, searchAbnoName} from "@/api/api";
  ...
  methods: {
    ...
    submitForm(){
      this.$refs.ruleForm.validate((valid)=>{
        if(valid){
          // 后续对表单数据的处理
          //调用新增API  post
          postAbno(this.ruleForm).then(response=>{
    ...
    handleDelete(row){
      this.$confirm('此操作将删除该记录, 是否继续?', '提示', {
        ...
        // console.log('rowid',row.id,row.abnormal_id)
        deleteAbn(row.id).then(response=>{
          console.log(response)
          this.getData()
        })
        this.$message({
